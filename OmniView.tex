...
By using the first option a drop down menu appears where the connected device and the loaded data is shown. It is designed after the design principle \ref{cap:Designprinciples_dropDownMenusWithCheckmark}. The user can choose which device or loaded data (waveform) should be send by clicking the checkbox mark next to the device or loaded data. The user can only send one waveform. 
